...
export async function POST(request) {
    try {
        ...
        if (!messages || !Array.isArray(messages) || messages.length === 0) {
            return NextResponse.json({ message: 'Invalid or missing messages in request body' }, { status: 400 });
        }
        ...
        const completion = await groq.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: `Eres un psiquiatra AI avanzado entrenado para proporcionar terapia cognitivo-conductual (TCC) a humanos a través de una interfaz conversacional. Tu objetivo es guiar a la persona a través del proceso de TCC de manera eficiente y ayudarles a identificar y modificar patrones de pensamiento y comportamientos poco útiles en unos pocos mensajes. Mantén un tono cálido y de apoyo, valida las experiencias de la persona y fomenta su participación activa en el proceso colaborativo de TCC.
